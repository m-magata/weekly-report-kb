...
- バッチは `auto_upload.py` を**2回**実行する
  - PASS 1: `python auto_upload.py` — 未登録ファイルのみ取り込み（過去月）
  - PASS 2: `python auto_upload.py --force --month <当月>` — 当月を強制上書き
  - 当月は各店がファイルを追記中で、一度登録されると通常実行では DUP スキップされ
    以降の追記が DB に反映されないため、当月のみ毎回上書きする
  - 当月は PowerShell の `(Get-Date).Month` から取得
  - どちらかの PASS が失敗したら非0で終了する
  - 所要時間の実測: 約2分（PASS1 249件チェック + PASS2 当月30件の再パース）
...
- 動作確認済み: 手動実行で exit code 0（pass1=0 pass2=0）

### 2026年6月分の取りこぼしを修正
- `auto_upload.py` に `--month N [N ...]` オプションを追加
  - 対象月を明示指定する（例: `--month 6` / `--month 6 7`）
  - 指定時は「当月まで」の制限を適用しない（過去分の再取り込みが用途のため）
- 6月分がファイル完成前に取り込まれており、週報③④の本文が DB に入っていなかった
  - 実ファイルには本文があるのに DB は40文字（`＜人員管理報告…＞` `＜来期へのコメント…＞` の見出しのみ）
  - 一度登録されると通常実行では DUP スキップされるため、追記が反映され続けていなかった
  - `python auto_upload.py --force --month 6` で再取り込み（31件 OK・エラー0件）
  - 結果: 週報②の中央値 61→1,322文字、週報③ 40→1,279文字、週報④ 40→1,297文字
  - ★行総数 45→157、★を含むレコード 19.1%→58.6%（5月79.0%・7月63.1%と同水準に回復）
- 1〜7月を検証した結果、**取りこぼしは6月のみ**だった
  - 他の月は週報①〜④とも中央値1,200〜1,380文字で、実質空のレコードは0件
  - 週報⑤⑥が40文字なのは正常（期首/期末週。3月・5月は5週あるため週報⑤も本文あり）
- 8月は取りこぼしではなく**記入途中**
  - `--force` で再取り込みしても変化は1件のみ、★行数も54で不変
  - 以降は PASS 2 で毎回上書きされるため、追記は自動で反映される
